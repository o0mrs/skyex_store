...
<div class="form-control w-full max-w-xs">
  ...
  <button class="text-gray-500 pl-2 pr-1">
    ...
    <input type="text" placeholder="Search for products" class="border-0 input focus:outline-none h-12 -mt-2 border-transparent focus:border-transparent focus:ring-0  bg-gray-100" value={searchin} onChange={(e)=>{setsearchin(e.target.value)}}/>
...
{catagorys.map((git) => {
  if(activetab == git.id){
    return(
      <div class="  tab snap-normal scrollbar-hide snap-center shrink-0  text-black border-b border-black" onClick={()=>{setactivetab(git.id)}}>{git.name}</div> 
    )
  }
  return(
    <div class="tab snap-normal scrollbar-hide snap-center shrink-0 " onClick={()=>{setactivetab(git.id)}}>{git.name}</div>
  )
})}
</div>
</div>
