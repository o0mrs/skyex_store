...
const Checkout = ()=>{
    ...
    const [selecctedshippingmetod,setselecctedshippingmetod] = useState(-1)
// 
//payments
...
axios.get(window.$api + '/phone').then((Response)=>{
  setphoneh(Response.data[0].logo)
    ...
    const checkinfo = ()=>{
      ...
      if(needshipping == 0){
        if(name.length < 2){
          setnewalart([...newalart,{id:2,name:"That name doesn't sound like a real one"}])
        }else if(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(email) == false || email.length < 3){
          setnewalart([...newalart,{id:2,name:"Hmm… that email doesn't look valid"}])
        }else{
          settab('loading3')
        }
      }else{
        if(name.length < 2){
          ...
          setnewalart([...newalart,{id:2,name:"Please select a country"}])
        }else if (address.length < 6){
          setnewalart([...newalart,{id:2,name:"Make sure your address is at least 6 characters"}])
        }else if (city.length < 3){
          setnewalart([...newalart,{id:2,name:"I don't think that's a real city"}])
        }else if (phoneh == 1 && phone.length < 7 && result){
          setnewalart([...newalart,{id:2,name:"The phone number doesn’t fit the rule."}])
          
        }else if (zipcode.length < 3){
          setnewalart([...newalart,{id:2,name:"Hmm...Your ZIP code doesn't look vaild"}])
        }else if(state.length == 0){
          setnewalart([...newalart,{id:2,name:"Please select a state"}])
        }else{
          settab('loading3')
        }
      }
...
{newalart.map((gf,i)=>{
  setTimeout(() => {
    setnewalart((products) => products.filter((_, index) => index !== i));
  }, 3000);
  return(
    <div class="alert  alert-warning shadow-lg max-w-xs mt-2 float-right proalt rounded-r-none text-left">
    <div>
  ...
  <span onClick={()=>{settab('sum')}} className='float-left cursor-pointer roboto'><i class="fa-solid fa-cart-shopping mr-1 text-left"></i>  Show order summary</span>
...
<label class="label  text-gray-700">
  ...
  <label class="label mt-2 text-gray-700">
...
<span className='text-white'>SubTotal: {subtotla} $</span>
  </div>
  <div className="form-control grid place-items-center">
<span className='text-white'>Shipping: {shippingprice}$</span>
  </div>
  <div className="form-control grid place-items-center">
<span className='text-white'>Total: {totalprice}$</span>
  </div>
  <div className="form-control grid place-items-center">
...
<div className='rounded-lg pt-4 mt-6 bg-base-200 shadow-sm  grid  sm:min-w-[90vw] lg:min-w-[30rem] '>
...
if(gfff.en == 1){
  return(
    <div className='w-full mt-2 emo text-left rounded-lg shadow-lg py-3   btn text-white bg-base-300'>
    
    <div className='roboto w-full'>
          <i class={"mr-2 " + gfff.icon}></i>
            <span>{gfff.name}</span>
  ...
  return(
    ...
    <div className='roboto rounded-full font-bold uppercase w-full'>
          <i class={"mr-2 " + gfff.icon}></i>
            <span>{gfff.name}</span>
            <span className='float-right'><i class="fa-solid fa-x text-red-500"></i></span>
            </div>
    </div>
        )
}
