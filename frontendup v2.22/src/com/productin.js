...
{alartstate.length > 0 && alartstate == 'Item Has been added successfully' && (
  <div class="alert ease-in-out duration-300 delay-75	 alert-success shadow-lg max-w-xs float-right proalart rounded-r-none text-left top-[1rem] md:top-[3.4rem]">
  <div>
...
{alartstate.length > 0 && alartstate !== 'Item Has been added successfully' && (
  <div class="alert alert-warning shadow-lg max-w-xs float-right proalart rounded-r-none text-left top-[1rem] md:top-[3.4rem]">
  <div>
...
if(g.enable == true){
  ...
  <div className=" mt-5 text-2xl flex justify-center">
    <div className="flex">
    <i class="fa-solid fa-pen-ruler -ml-10"></i>
  ...
  {coptions.map((g,i)=>{
    ...
    if(g.enable === true){
      ...
      if(g.instock > 0){
        if(i == coption.arnum){
          return(
            <button key={i+1} className="btn text-white m-2  ring ring-cyan-800" onClick={()=>{
              setcoption({arnum:i,name:g.optionName})
              setcoprice(g.price)
            }}>{g.optionName}</button> 
          )
          ...
          return(
            <button key={i+1} className="btn text-white m-2  " onClick={()=>{
              setcoption({arnum:i,name:g.optionName})
              setcoprice(g.price)
            }}>{g.optionName}</button> 
          )
...
<button className="btn text-white m-2 ">{g.optionName}</button> 
</div>
...
<div className="grid m-6 ml-12 -mt-10 ">
    ...
    <div id='flavoursContainer' className="w-full overflow-scroll grid place-items-center p-4 carousel">
          <div id='flavoursContainer' className="relative max-w-[22rem]  min-w-[21rem] min-h-[20rem]   grid place-items-center  mt-16 carousel-item">
              {imgss.length > 0 && (
                <div>
...
<div className="grid grid-cols-4 w-96 ml-4">
    {imgss.map((git,i)=>{
...
<div className="text-4xl roboto">{name}</div>
...
<div className="flex mt-10 grid-cols-5">
  ...
  {sizes.map((g,i)=>{
    if(g.enable == true){
      if(g.instock > 0){
        if(i == size.arnum){
          return(
            <button className="ring text-white ring-cyan-800 m-2 btn btn-circle btn-md" onClick={() =>{
              setsize({arnum:i,name:g.name})
              setsprice(g.price)
            }}>{g.name}</button> 
          )
          ...
          return(
            <button className="m-2 text-white btn btn-circle btn-md" onClick={() =>{
              setsize({arnum:i,name:g.name})
              setsprice(g.price)
            }}>{g.name}</button> 
          )
        ...
        return(
          <div data-tip="out of stock" className="tooltip">
          <button className="btn text-white btn-circle btn-md m-2" disabled="">{g.name}</button> 
</div>
  ...
  {coptions.map((g,i)=>{
    ...
    if(g.enable === true){
      ...
      if(g.instock > 0){
        if(i == coption.arnum){
          return(
            <button className="btn text-white m-2  ring ring-cyan-800" onClick={()=>{
              setcoption({arnum:i,name:g.optionName})
              setcoprice(g.price)
            }}>{g.optionName}</button> 
          )
          ...
          return(
            <button className="btn text-white m-2  " onClick={()=>{
              setcoption({arnum:i,name:g.optionName})
              setcoprice(g.price)
            }}>{g.optionName}</button> 
          )
...
<button className="btn btn-active text-white  mr-3" onClick={()=>{
  handle_add_cart()
