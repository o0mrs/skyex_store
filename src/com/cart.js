...
//payments
  ...
  <span  className='float-left cursor-pointer roboto'><i className="fa-solid fa-cart-shopping mr-1 text-left"></i>  Checkout</span>
...
{price} {currency}
  ...
  <ul>
  <li onClick={()=>{settabs('sum')}} className="font-[450] text-cyan-800 cursor-pointer ">Summary</li> 
    <li onClick={()=>{settabs('app')}} className="font-[450] cursor-pointer ">Account</li> 
    <li onClick={()=>{settabs('ship')}} className="font-[450]">Shipping</li> 
...
<div>
  <button className="btn btn-outline  text-base-300" onClick={()=>{
...
<div className=" px-16 py-6 rounded-xl bg-base-300 Poppins text-white shadow-md sm:min-w-[80vw] lg:min-w-[40rem]">
        <div className='Poppins text-xl -mx-12  font-bold mt-6"'>
          Summary
          </div>
<div className='-mx-12 mt-4 Poppins'>
{cart.map((g,i)=>{
    console.log(g)
    ...
    if (ff.status == 405){
     ...
     <div data-tip="OUT OF STOCK" className="tooltip  tooltip-error -ml- -mr-4"> <span className='mt-2'>    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="#2196f3" className="w-6 h-6 ">
      ...
      return(
        ...
        <span>{ff.name}</span><span className='ml-4 float-right text-cyan-600'> {g.price} {currency}</span>
        </div>
      )
...
<div className=" mt-4 -mx-12 ">
  <span className='Poppins font-bold'>
    SubTotal<div className='text-right -mt-6 text-cyan-600'> {price} {currency}</div>
