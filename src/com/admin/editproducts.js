...
const [test,settest] = useState([{name:'XS',vid:"f", enable:true,price:2,instock:3}])
...
const [sizevid,setsizevid] = useState('')
...
if(tab == 'app'){
   ...
   <div className='bg-black -mt-4 pt-5 pb-10' id='d99c' >
        {/* sm: md: lg: xl: */}
        {newalart.length > 0 &&(
        <div className="alert absolute alert-warning shadow-lg max-w-xs float-right proalart rounded-r-none text-left top-[2rem] md:top-[3.4rem]">
        <div>
        ...
        <span className='min-w-[15rem] '>{newalart}</span>
                ...
                {suck.length > 0 &&(
        <div className="alert absolute alert-success shadow-lg max-w-xs float-right proalart rounded-r-none text-left top-[2rem] md:top-[3.4rem]">
        <div>
          <svg xmlns="http://www.w3.org/2000/svg" className="stroke-current flex-shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
...
{/* size */}
<div className={'grid place-items-center mt-3  overflow-x-auto ' + sizcheck}>
...
<th>
<input type="text" value={sizevid} placeholder='VID' onChange={(e)=>{setsizevid(e.target.value)}} className="w-24 h-9 input input-accent input-bordered bg-base-300/[0.4] focus:ring-black mt-2" />
</th>
<th onClick={()=>{
if(sizeaddname === '' || sizeaddprice === '' || sizeaddinstock === ''){
  ...
  settest([...test,{name:sizeaddname,enable:true,vid:sizevid,price:sizeaddprice,instock:sizeaddinstock}])
  setsizeaddname('')
  setsizeaddprice('')
  setsizeaddinstock('')
}
