...
const Add_region = ({setbar, bar,setTabo,shippingto})=>{ 
...
if(name.length < 1){
    ...
    axios.post(window.$api + 'add_region',{
              userToken: userToken,
...
<div id='c3' className="bg-gray-900 h-16 w-screen pl-5 text-gray-200 py-5 text-xl">Add Region<span onClick={()=>{
...
{countryListAllIsoData.map((g,i)=>{
  ...
  <option disabled="disabled" selected="selected">Choose</option> 
  <option value={2}>Countain states</option> 
  <option value={1}>without states</option> 
  ...
  <div className={"form-control " + prices}>
    ...
    <input value={price} onChange={(e)=>{setprice(e.target.value.replace(/[A-z]/g, ''))}} type="text" placeholder="0" className="input text-center" />
  </div>
...
<button onClick={()=>{
  ...
  <button className="btn btn-active" role="button" aria-pressed="true" onClick={handlesubmit}>Add Region</button> 
  </div>
